...
mfb_set_viewport_best_fit(struct mfb_window *window, unsigned old_width, unsigned old_height) {
    if(window != 0x0) {
        ...
        unsigned int finalWidth  = (unsigned int)((old_width  * scale_x) + 0.5f);
        unsigned int finalHeight = (unsigned int)((old_height * scale_y) + 0.5f);

        unsigned int offset_x = (new_width  - finalWidth)  >> 1;
        unsigned int offset_y = (new_height - finalHeight) >> 1;

        mfb_get_monitor_scale(window, &scale_x, &scale_y);
        return mfb_set_viewport(window, (unsigned int)(offset_x / scale_x), (unsigned int)(offset_y / scale_y), (unsigned int)(finalWidth / scale_x), (unsigned int)(finalHeight / scale_y));
    }
